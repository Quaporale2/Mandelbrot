...
// Retire une vue de l'historique
...
// Dessine la texture du Mandelbrot dans un objet texture SDL
void draw_mandelbrot_normal(SDL_Renderer *renderer, SDL_Texture *texture, int max_iteration, double zoom, double offsetX, double offsetY, int width, int height, bool antialiasing) {
    ...
    for (int py = 0; py < height; py++) {
        for (int px = 0; px < width; px++) {
            double x0 = (px - width / 2.0) / zoom + offsetX;
            double y0 = (py - height / 2.0) / zoom + offsetY;
...
int main(int argc, char *argv[]) {
    ...
    // Redessine l'interface, et recalcule le mandelbrot si le zoom ou l'offset à changé, sauf si autorefresh désactivé
    bool redrawAllIfMovement = false;
    // Redessine forcément l'interface et recalcule le mandelbrot, sauf si autorefresh désactivé
    bool redrawAllAuto = false;
    // Si l'utilisateur demande manuellement un redessin, on redessine l'interface et on recalcule le Mandelbrot
    bool redrawAll = true;
    ...
    SDL_Window *window = SDL_CreateWindow("Fractale Mandelbrot", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, windowWidth, windowHeight, SDL_WINDOW_RESIZABLE);
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            // Clic molette recalcule le mandelbrot
            if (event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_MIDDLE && !menuMode && initialClickDone) {
                redrawAll = true;
            }
        ...
        */
        ...
        if ((redrawInterface || redrawAllIfMovement || redrawAllAuto || redrawAll) && !rightDragging) {
        
            // Si on effectue un zoom ou qu'on force le recalcul
            if (((((zoom != lastZoom || offsetX != lastOffsetX || offsetY != lastOffsetY) && redrawAllIfMovement) || redrawAllAuto) && activateAutoRefresh) || redrawAll) {
                ...
                // Sélectionner la texture comme cible
                SDL_SetRenderTarget(renderer, fractalTexture);
                
                /*
                pthread_t fractalThread;
                if (pthread_create(&fractalThread, NULL, compute_fractal, (void*)&params) != 0) {
                    fprintf(stderr, "Erreur lors de la création du thread.\n");
                    return 1;
                }
                */
                
                // Calculer et dessiner la fractale sur la texture existante        
                // Version complexe seulement dans la version linx
                #ifdef __linux__
                    if (advancedMode) {
                        draw_mandelbrot_high_precision(renderer, fractalTexture, max_iteration, zoom, offsetX, offsetY, windowWidth, windowHeight, activateAntialiasing);
                    } else {
                        draw_mandelbrot_normal(renderer, fractalTexture, max_iteration, zoom, offsetX, offsetY, windowWidth, windowHeight, activateAntialiasing);
                    }
                #else
                    draw_mandelbrot_normal(renderer, fractalTexture, max_iteration, zoom, offsetX, offsetY, windowWidth, windowHeight, activateAntialiasing);
                #endif
            ...
            // Va zoomer sur la texture sans recalcul
            } else {
            ...
            if (!hideInterface) {
                ...
                render_text(renderer, font, displayBuffer, 10, windowHeight - 30, ORIGIN_UP_LEFT);
                sprintf(displayBuffer, "Zoom actuel: %f", zoom);
                render_text(renderer, font, displayBuffer, 10, windowHeight - 60, ORIGIN_UP_LEFT);
                sprintf(displayBuffer, "Offset actuel: X: %f   Y: %f", offsetX, offsetY);
                render_text(renderer, font, displayBuffer, 10, windowHeight - 90, ORIGIN_UP_LEFT);
                
                // Controles, bord bas droite
                // Seulement dans la version linux
                #ifdef __linux__
                    if (advancedMode) {
                        render_text(renderer, font, "M pour toggle précision Normale/Haute:   HAUTE", windowWidth - 10, windowHeight - 10, ORIGIN_DOWN_RIGHT);
                    } else {
                        render_text(renderer, font, "M pour toggle précision Normale/Haute: NORMALE", windowWidth - 10, windowHeight - 10, ORIGIN_DOWN_RIGHT);
                    }
                #endif
                if (activateAntialiasing) {
                    render_text(renderer, font, "J pour toggle l'antialiasing:  ON", windowWidth - 10, windowHeight - 40, ORIGIN_DOWN_RIGHT);
                } else {
                    render_text(renderer, font, "J pour toggle l'antialiasing: OFF", windowWidth - 10, windowHeight - 40, ORIGIN_DOWN_RIGHT);
                }
                if (activateAutoRefresh) {
                    render_text(renderer, font, "R pour toggle l'autorefresh:  ON", windowWidth - 10, windowHeight - 70, ORIGIN_DOWN_RIGHT);
                } else {
                    render_text(renderer, font, "R pour toggle l'autorefresh: OFF", windowWidth - 10, windowHeight - 70, ORIGIN_DOWN_RIGHT);
                }
                switch (colorScheme) {
                    case HOT_COLD:
                        render_text(renderer, font, "B pour alterner les couleurs: CHAUD/FROID", windowWidth - 10, windowHeight - 100, ORIGIN_DOWN_RIGHT);
                        break;
                    case WHITE_BLACK:
                        render_text(renderer, font, "B pour alterner les couleurs:  BLANC/NOIR", windowWidth - 10, windowHeight - 100, ORIGIN_DOWN_RIGHT);
                        break;
                }
                render_text(renderer, font, "H pour toggle l'interface", windowWidth - 10, windowHeight - 130, ORIGIN_DOWN_RIGHT);
                render_text(renderer, font, "I pour entrer une nouvelle valeur d'itérations max", windowWidth - 10, windowHeight - 160, ORIGIN_DOWN_RIGHT);
                render_text(renderer, font, "Espace pour recalculer l'image du Mandelbrot", windowWidth - 10, windowHeight - 190, ORIGIN_DOWN_RIGHT);
                render_text(renderer, font, "Clic droit simple pour retour en arrière", windowWidth - 10, windowHeight - 220, ORIGIN_DOWN_RIGHT);
                render_text(renderer, font, "Clic droit/gauche glissé & flèches directionnelles pour naviguer", windowWidth - 10, windowHeight - 250, ORIGIN_DOWN_RIGHT);
            }
